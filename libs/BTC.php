...
class BTC
{
    static private $thisCurrency = 'BTC';
    static private $lastError = false;

    static private function node($method, array $params = [])
    {
        $ch = curl_init();
        curl_setopt($ch, CURLOPT_URL, BTC_IP);
        curl_setopt($ch, CURLOPT_PORT, BTC_PORT);
        curl_setopt($ch, CURLOPT_HTTPAUTH, CURLAUTH_BASIC);
        curl_setopt($ch, CURLOPT_USERPWD, BTC_USER . ':' . BTC_PASS);
    ...
    static public function addAddress()
    {
        global $DB;
        global $User;
        $Address = self::node('getnewaddress', [BTC_ACCOUNT]); echo self::getLastError();

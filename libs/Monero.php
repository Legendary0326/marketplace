...
class Monero 
{
    ...
    public function callMonero($fn,$params=array())
    {
        $ch = curl_init();
                $data = array(
                            'jsonrpc'       => '2.0', 
      ...
      public function callWithoutRPC($fn,$params=array())
    {
        $ch = curl_init();
                $data = array(
                            'jsonrpc'       => '2.0', 
        ...
        // Set up the cURL RPC request
                curl_setopt($ch, CURLOPT_URL, 'http://' . $this->rpc_address.'/'.$fn);
                curl_setopt($ch, CURLOPT_POST, 1);
                curl_setopt($ch, CURLOPT_ENCODING, 'Content-Type: application/json');
                curl_setopt($ch, CURLOPT_POSTFIELDS,json_encode($data));
                curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
    ...
    {      
        ...
        if ($pending = $this->mysqli->prepare('SELECT `payment_id` FROM `xmr_payments`
            ...
            if ($pending->num_rows > 0) {
                ...
                $server_output = curl_exec ($ch);
                echo "<hr />";
                echo 'http://' . $this->rpc_address . '/json_rpc';
                echo "<hr />";
                print_r($data);

<?php /***REALFILE: /var/www/vhosts/market1304.de/httpdocs/nav/cat_search.php***/
...
?><style>
#catShowHide_down + label::before { content: "alle Kategorien"; }
#catShowHide_down:checked + label::before { content: "alle Kategorien"; }
#searchShowHide_down + label::before { content: "Suchoptionen ▼"; }
#searchShowHide_down:checked + label::before { content: "Suchoptionen ▲"; }
</style>
...
echo '<div><a style="color: #495057; margin-left: -1.8%;" href="/marketplace?setCat=ALL"><span>alle Kategorien</span><span style="margin-left: 25px; color: #007bff;">(' . Categories::numAllCategories() . ')</span></a></div><br>' . nl;
...
?>
            ...
            <div class="col-md-6 p-1">
                <div class="input-group" style="margin-bottom: -20px;">
                    <input type="text" class="form-control" placeholder="Suchbegriff" name="searchterm" value="<?php echo $Market->get('searchterm'); ?>">
                    <div class="input-group-append">
                        <button class="btn-sm btn-secondary ml-2" type="submit">Suche</button>
                    </div>
                </div>
                <br>
...
?>
                        ...
                        <div class="form-group">
                            <span class="float-left pt-2">von&nbsp;</span><input type="text" class="form-control w-25 float-left" placeholder="0.00" name="priceFrom" value="<?php echo (empty($Market->get('priceFrom')) ? '' : $Language->number($Market->get('priceFrom'), 2)); ?>"><span class="float-left pt-2">&nbsp;bis&nbsp; </span><input type="text" class="form-control w-25 float-left" placeholder="9999.99" name="priceTill" value="<?php echo (empty($Market->get('priceTill')) ? '' : $Language->number($Market->get('priceTill'), 2)); ?>"><span class="float-left pt-2">&nbsp;<?php echo $User->get('Currency'); ?></span>
                        </div>
                        <br>
                        <br>
                        <br>
                        <br>
                        min. Status Verkäufer:
                        <br>
                        <br>
                        <div>
...
?>
                        ...
                        <div>
                            <select class="form-control" name="orderBy">
                                <option value="priceASC"<?php echo Forms::selectedVal($Market->get('orderBy'), 'priceASC'); ?>>Preis aufsteigend</option>
                                <option value="priceDESC"<?php echo Forms::selectedVal($Market->get('orderBy'), 'priceDESC'); ?>>Preis absteigend</option>
                                <option value="dateASC"<?php echo Forms::selectedVal($Market->get('orderBy'), 'dateASC'); ?>>Datum aufsteigend</option>
                                <option value="dateDESC"<?php echo Forms::selectedVal($Market->get('orderBy'), 'dateDESC'); ?>>Datum absteigend</option>
                            </select>
                        </div>
                    </div>
                </aside>

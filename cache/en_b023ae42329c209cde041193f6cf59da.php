<?php /***REALFILE: /var/www/vhosts/market1304.de/httpdocs/pages/bitcoinwallet.php***/
...
if ($Pages->inPath('withdrawalok')) {
    ...
    if (Captcha::verify()) {
        if ($User->get('WC') == $_POST['WDC']) {
            if (BTC::checkAddress($_POST['Address'])) {
                ...
                if ($UserAmount > 0) {
                    if ($User->hasRole('admin')) {
                        ...
                        $Amount = $User->get('BTC');
                        $SQLResult = $DB->query('SELECT SUM(Amount) AS AmountSum FROM crypto_transactions WHERE User = ' . $DB->int($User->getID()) . ' AND Currency = ' . $DB->string('BTC') . ' AND Received >= ' . $DB->int(time() - 86400));
                        if (!empty($SQLResult)) {
                    ...
                    $USD = Currencies::exchange($Amount - $UserAmount, 'BTC', 'USD', false);
                    if ($USD >= BTC_MINUSD) {
                        if ($Amount >= $UserAmount) {
                            if (BTC::sendMoney($_POST['Address'], $UserAmount)) {
                                $NewData = [
                                    'User'          =>  $User->getID(),
                                    'Currency'      =>  'BTC',
                                    'Date'          =>  time(),
                                ...
                                if ($User->hasRole('admin')) {
                                    $DB->query('UPDATE system_accounts SET BTC = BTC - ' . $DB->float($UserAmount) . ' WHERE SAccountID = ' . $DB->string('fee'));
                                } else {
                                    $DB->query('UPDATE user SET BTC = BTC - ' . $DB->float($UserAmount) . ' WHERE UserID = ' . $DB->int($User->getID()));
                                }
                        ...
                        echo Alerts::danger(sprintf('The Balance On Your Account Is Too Low! (Min. Balance = %.2f US-$!)', BTC_MINUSD), 'mt-3');
                    }
                } else {
...
<div class="container mt-3">
    <div class="row py-1">
        <div class="col-md-12 text-info">
            <h3>Your Personal Bitcoin Wallet...</h3>
        </div>
    </div>
</div>
...
<div class="container mt-3">
    <div class="row py-1">
        <div class="col-md-12">
            Here you can manage your Bitcoin balance as well as deposit and withdraw funds. To deposit funds, send Bitcoins to the integrated address below (klick button [Create deposit address]). The balance will appear after 3 blockchain confirmations, which should complete your payment in about 30 minutes. The address will change between each deposit, so please always deposit to the latest address. You can deposit any amount you want. All marketplace internal transactions are absolutely free of fees. For deposits and withdrawals of Bitcoins, you have to pay the usual fees, charged by the blockchain.
            <br><br>
            ...
            <br><br>
            <span class="text-danger">If you send Bitcoins to an unvalid address, they might not be credited properly!</span>
        </div>
    </div>
</div>
...
if ($Pages->inPath('create')) {
    $Address = BTC::addAddress();
?>
<div class="container mt-3">
    <div class="row">
        <div class="col-md-12">
            <img class="mx-auto d-block" src="<?php
echo 'data:image/svg+xml;base64,' . base64_encode(QRcode::svg('bitcoin:' . $Address, md5($Address), false, QR_ECLEVEL_L, false, false, 0));
...
<div class="container mt-3">
    <div class="row">
        <div class="col-md-12">
            <a class="btn btn-primary btn-block mx-auto d-block" style="width: 33%; min-width: 250px;" href="/bitcoinwallet/create">Create Deposit Address</a>
        </div>
    </div>
</div>
...
<?php
$Transactions = BTC::getLastReceived();
...
<div class="container mt-3">
    <div class="row py-1">
        ...
        <div class="col-md-12">
            ...
            <br>
            Enter a valid Bitcoin address and the amount you wish to withdraw below.
            <br><br>
            <span class="text-danger"><?php printf('Please note!<br>Amounts less than %.2f US-$ cannot be paid out for security reasons (deposit for blockchain transaction fees). Amounts that you have received from an external wallet address can be paid out at least 24 hours after they have been credited!', BTC_MINUSD); ?></span>
        </div>
    </div>
</div>
...
?>
<form method="post" action="/bitcoinwallet">
    <div class="container mt-3">
        <div class="row py-1">
    ...
    <div class="container mt-0">
        <div class="row py-1">
            ...
            <div class="col-md-8">
                <input type="text" class="form-control float-left w-50" name="Amount" placeholder="0.00000000"><span class="float-left ml-2 pt-1">BTC</span>
            </div>
        </div>
    </div>
...
<?php
$Withdrawals = BTC::getLastWithdrawals();
